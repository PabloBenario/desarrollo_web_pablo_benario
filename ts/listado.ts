...
(function () {
  const tbody = document.getElementById("tbody-miembros") as HTMLTableSectionElement;
  const filtroTipo = document.getElementById("filtro-tipo") as HTMLSelectElement;
  const btnPrev = document.getElementById("pag-prev") as HTMLButtonElement;
  const btnNext = document.getElementById("pag-next") as HTMLButtonElement;
  const pagInfo = document.getElementById("pag-info") as HTMLElement;
  const detalleCont = document.getElementById("detalle-miembro") as HTMLElement;
  const ths = Array.from(
    document.querySelectorAll<HTMLTableCellElement>("th.ordenable")
  );
  ...
  function obtenerValor(m: Miembro, col: string): string | number {
    switch (col) {
      case "nombre":
  ...
  function filtradosOrdenados(): Miembro[] {
    let arr = MIEMBROS.slice();
    if (filtro) arr = arr.filter((m) => m.tipo === filtro);
    arr.sort((a, b) => {
  ...
  function renderizar(): void {
    ...
    ths.forEach((th) => {
      ...
      if (th.dataset.col === columna) {
        ...
        if (icono) icono.textContent = direccion === "asc" ? "↑" : "↓";
      }
    });
  }

  function mostrarDetalle(m: Miembro): void {
    let info = "";
      ...
      <article class="detalle-actividad">
        ...
        <ul>
          ${a.horarios
            .map(
              (h) =>
                `<li>${ETIQUETAS_DIA[h.dia]} de ${h.horaInicio} a ${h.horaFin}</li>`
            )
            .join("")}
        </ul>
    ...
    if (cerrar) cerrar.addEventListener("click", () => (detalleCont.hidden = true));
