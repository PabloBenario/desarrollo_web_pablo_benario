...
(function () {
  ...
  const fsEstudiante = document.getElementById("datos-estudiante") as HTMLFieldSetElement;
  const fsFuncionario = document.getElementById("datos-funcionario") as HTMLFieldSetElement;
  const fsAcademico = document.getElementById("datos-academico") as HTMLFieldSetElement;
  const lblTelegram = document.getElementById("label-telegram") as HTMLElement;
  const lblTelefono = document.getElementById("label-telefono") as HTMLElement;
  const listaActividades = document.getElementById("lista-actividades") as HTMLElement;
  const btnAgregarActividad = document.getElementById("btn-agregar-actividad") as HTMLButtonElement;
  const mensaje = document.getElementById("mensaje-resultado") as HTMLElement;

  let contadorActividad = 0;

  const DIAS: Array<[DiaSemana, string]> = [
    ["lunes", "Lunes"],
    ["martes", "Martes"],
    ["miercoles", "Miércoles"],
    ["jueves", "Jueves"],
  ...
  function alternarCamposEspecificos(): void {
  ...
  function crearFilaHorario(): HTMLElement {
    ...
    `;
    const btnEliminar = fila.querySelector(".btn-eliminar-horario") as HTMLButtonElement;
  ...
  function crearBloqueActividad(): HTMLElement {
    contadorActividad++;
    ...
    const listaHorarios = section.querySelector(".lista-horarios") as HTMLElement;
    listaHorarios.appendChild(crearFilaHorario());

    const btnEliminarAct = section.querySelector(".btn-eliminar-actividad") as HTMLButtonElement;
    btnEliminarAct.addEventListener("click", () => {
      ...
      if (bloques.length > 1) {
        ...
        const errAct = document.getElementById("err-actividades") as HTMLElement;
        errAct.textContent = "Debe haber al menos una actividad";
        window.setTimeout(() => (errAct.textContent = ""), 3000);
      }
    });

    const btnAgregarHor = section.querySelector(".btn-agregar-horario") as HTMLButtonElement;
  ...
  function renumerarActividades(): void {
    const bloques = listaActividades.querySelectorAll<HTMLElement>(".actividad-bloque");
    bloques.forEach((b, i) => {
      const h4 = b.querySelector("h4");
      if (h4) h4.textContent = `Actividad #${i + 1}`;
    });
  }
  ...
  function validarActividad(
    bloque: HTMLElement,
    idx: number,
    todosHorarios: Array<{ h: Horario; idx: number }>
  ): boolean {
    const nombreAct = (bloque.querySelector(".act-nombre") as HTMLInputElement).value.trim();
    const categoria = (bloque.querySelector(".act-categoria") as HTMLSelectElement).value;
    const archivosInput = bloque.querySelector(".act-archivos") as HTMLInputElement;
    const enlace = (bloque.querySelector(".act-enlace") as HTMLInputElement).value;

    const errNombre = bloque.querySelector(".act-err-nombre") as HTMLElement;
    const errCategoria = bloque.querySelector(".act-err-categoria") as HTMLElement;
    const errArchivos = bloque.querySelector(".act-err-archivos") as HTMLElement;
    const errEnlace = bloque.querySelector(".act-err-enlace") as HTMLElement;
    const errHorarios = bloque.querySelector(".act-err-horarios") as HTMLElement;
    ...
    if (!archivos || archivos.length === 0) {
      ...
      const invalido = Array.from(archivos).find(
    ...
    const filas = Array.from(bloque.querySelectorAll<HTMLElement>(".grupo-horario"));
    let mensajeHorario = "";
    filas.forEach((fila) => {
      const dia = (fila.querySelector(".h-dia") as HTMLSelectElement).value as DiaSemana | "";
      const ini = (fila.querySelector(".h-inicio") as HTMLInputElement).value;
      const fin = (fila.querySelector(".h-fin") as HTMLInputElement).value;
      if (!dia || !ini || !fin) {
  ...
  function detectarTraslapes(todosHorarios: Array<{ h: Horario; idx: number }>): boolean {
    const errTras = document.getElementById("err-traslape") as HTMLElement;
  ...
  function validar(): boolean {
    ...
    // Actividades
    const bloques = Array.from(listaActividades.querySelectorAll<HTMLElement>(".actividad-bloque"));
    const todosHorarios: Array<{ h: Horario; idx: number }> = [];
