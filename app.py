...
from models import Actividad, Comentario, Comuna, Foto, Horario, Miembro, Region, db
from validators import (
    ...
    validar_comentario_nombre,
    validar_comentario_texto,
    validar_correo,
    validar_hora_rango,
    validar_no_vacio,
    validar_nombre,
...
def api_estadisticas():
    ...
    return jsonify(
        {
            "miembrosPorDia": {
                "labels": [dia.strftime("%d-%m-%Y") for dia, _ in por_dia],
                "data": [total for _, total in por_dia],
            },
            "actividadesPorTipo": {
                "labels": [CATEGORIAS_ACTIVIDAD.get(tipo, tipo) for tipo, _ in por_tipo],
                "data": [total for _, total in por_tipo],
            },
            "actividadesPorComuna": {
                "labels": [nombre for nombre, _ in por_comuna],
                "data": [total for _, total in por_comuna],
            },
        }
    )


def comentario_json(comentario):
    return {
...
def comentarios(actividad_id):
    if db.session.get(Actividad, actividad_id) is None:
        abort(404)

    if request.method == "POST":
        datos = request.get_json(silent=True) or {}
        nombre = texto(datos.get("nombre"))
        contenido = texto(datos.get("texto"))
        errores = {}
        error = validar_comentario_nombre(nombre)
        if error:
            errores["nombre"] = error
        error = validar_comentario_texto(contenido)
        if error:
            errores["texto"] = error
        if errores:
        ...
        db.session.commit()
        return jsonify({"ok": True, "comentario": comentario_json(comentario)}), 201
    ...
    return jsonify([comentario_json(c) for c in lista])
